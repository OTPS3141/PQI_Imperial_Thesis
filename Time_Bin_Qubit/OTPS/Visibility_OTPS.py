...
def detection_binning(channels, timestamps, ch_AWG, bin_size = 25):

    times_shifted = align_triggers(channels, timestamps, ch_AWG)

    time_bin_list = []
    element = 0


    while element <= len(times_shifted):

        count = 0
        index = 0

        for i, elem in enumerate(times_shifted[element:]):
            if elem == 0:
                count += 1
            if count == bin_size+1:
                index = i
        ...
        time_detection = times_shifted[element:index+element]
        time_detection = time_detection[time_detection != 0]


        element += index
        # print(count, element, times_shifted[element])

        # small_bin_detection = times_shifted[0:index+1][times_shifted[0:index] != 0]
        # bin_list.append(small_bin_detection)



        time_bin_list.append(time_detection)
...
def build_histogram(channels,times, ch_trig, ch_det,binwidth,laser_rep,n_bins):
    ...
    time_bin_list = detection_binning(channels, times, ch_trig, bin_size=25)
    ...
    for time_bin in time_bin_list:
        counts,bins = np.histogram(time_bin,bins = n_bins)
    ...
    return t_hist, counts_list, time_bin_list
