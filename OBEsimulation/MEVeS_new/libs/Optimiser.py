...
class optimiser:
    ...
    def generate_filename(self):
        ...
        if self.directory != None:
            pkl_filename = f"\\\\?\\{self.directory}\\{current_datetime}.pkl"
            npz_filename = f"\\\\?\\{self.directory}\\{current_datetime}.npz"
        else:
            pkl_filename = f"{current_datetime}.pkl"
            npz_filename = f"{current_datetime}.npz"
        
        # Create empty files if don't exist
        if not os.path.exists(pkl_filename):
            with open(pkl_filename, "w+"):
                pass
        if not os.path.exists(npz_filename):
            with open(npz_filename, "w+"):
                pass
    
        return pkl_filename, npz_filename
    ...
    def arrays_for_counter_prop(self):
        ...
        elif self.sol.protocol == 'ORCA' or self.sol.protocol == 'TORCA' or self.sol.protocol == 'ORCA_GSM':
            # make coord grids for transform
            self.y = self.sol.t_grid + 2*self.sol.z_grid/self.sol.cNU # t_grid, y make up irregular grid 
            self.yline = np.linspace(0, self.sol.tbounds[-1]*self.sol.gamma+2/self.sol.cNU, self.sol.m)
            self._t_grid, self._y = np.meshgrid(self.sol.tpoints, self.yline) # _t_grid, _y make up regular grid 
    ...
    def storage_opt_GSM_mapping(self, Controls):
        if self.sol.protocol == 'ORCA_GSM':
            [Control, M1, M2] = Controls
            # counter-propagating
            # need to reset detunings
            self.sol.detunings(self.sol.deltaS, self.sol.deltaC)
            ...
            if self.sol.solved:
                ...
                if self.extra_params:
                    ...
                    eff_bothS = 1 #self.sol.storage_efficiency(self.sol.Sgs, -1) + self.sol.storage_efficiency(self.sol.Sgb, -1)
                    self.sol.E[:] = 0
                    self.sol.Pge[:] = 0
                    self.sol.Pge2[:] = 0
            ...
            if self.sol.solved:
                ...
                self.sol.Pge[:] = 0
                self.sol.Pge2[:] = 0
                self.sol.Sgs[0] = 0 #np.conj(np.flip(self.sol.Sgs[-1], axis=0))/np.sqrt(eff_bothS)
                self.sol.Sgs[1:] = 0
                self.sol.Sgb[0] = np.conj(np.flip(self.sol.Sgb[-1], axis=0))/np.sqrt(eff_bothS)
                self.sol.Sgb[1:] = 0
                self.sol.Einits = interp1d(self.sol.tpoints, np.zeros(self.sol.m)[:, None]*np.array([1, 0]), axis=0, fill_value="extrapolate", bounds_error=False)
                #normed *= eff_bothS
                self.sol.detunings(self.sol.deltaS, self.sol.deltaC)
                ...
                #                                 field=0)
                readin_grad_control = np.zeros((self.sol.m, 2))
                ...
                readin_grad_M2 = self.grad([Pge2copy, Sgbcopy], 
                                                ...
                                                M2/self.sol.gamma, 
                                                field=2)
                grads = np.array([readin_grad_control, readin_grad_M1, readin_grad_M2])
                total_eff = self.sol.retrieval_efficiency(self.sol.E, 0, 0) + self.sol.retrieval_efficiency(self.sol.E, 0, 1)
            else:
                storage_eff = 0
                total_eff = 0
                grads = 0
    ...
    def backward_retrieval_opt_MZ_storage(self, Controls):
        [eta_loop] = self.extra_params
        if self.sol.protocol == 'Raman':
            [Control1, Control2] = Controls
            # co-propagating
            Control_func = self.sol.co_prop(Control1) # func(t, z) => (p)
            self.sol.solve(Control_func, method=self.method)
            # need to save P and S coherences for gradient calculation
            Ecopy = np.array(self.sol.E)
            Scopy = np.array(self.sol.S)
            ...
            self.sol.Einits = interp1d(self.sol.tpoints, np.zeros(self.sol.m)[:, None]*np.array([1, 0]), axis=0, fill_value="extrapolate", bounds_error=False)
            # solve co-function storage (time and space reversed), must time reverse control
            Control_func = self.sol.co_prop(np.flip(np.conj(Control2), axis=0))
            self.sol.solve(Control_func, method=self.method)
            ...
            self.sol.S[0] = self.sol.S[-1]
            self.sol.S[1:] = 0
            Control_func = self.sol.co_prop(np.flip(np.conj(Control1), axis=0))
            self.sol.solve(Control_func, method=self.method)
            retrieval_eff = self.sol.retrieval_efficiency(self.sol.E, 0, 0) + self.sol.retrieval_efficiency(self.sol.E, 0, 1) # if spin wave normalised, this is retrieval efficiency
            total_eff = retrieval_eff * storage_eff
            grad = self.grad([Ecopy, Scopy], [np.conj(np.flip(np.flip(-self.sol.E, axis=0), axis=1)), np.conj(np.flip(np.flip(self.sol.S, axis=0), axis=1))], Control1/self.sol.gamma)
            # set up boundary conditions for next run
            self.sol.E[:] = 0
            self.sol.S[:] = 0
            self.sol.Einits = interp1d(self.sol.tpoints, self.sol._Einits/np.sqrt(self.sol.gamma), axis=0, fill_value="extrapolate", bounds_error=False) # initial photon condition, in natural units
    ...
    def forward_opt_MZ_all4(self, Controls):
        ...
        if self.sol.protocol == 'Raman':
            ...
            if self.sol.solved:
                ...
                Control_func4 = self.sol.co_prop(np.flip(np.conj(Control4), axis=0))
                self.sol.solve(Control_func4, method=self.method)
                grad4 = self.grad([Pcopy4, Scopy4], [np.conj(np.flip(np.flip(self.sol.P, axis=0), axis=1)), np.conj(np.flip(np.flip(self.sol.S, axis=0), axis=1))], Control4/self.sol.gamma)
            if self.sol.solved:
                # MZ store
                self.sol.Einits = interp1d(self.sol.tpoints, self.sol.E[:, -1]*np.sqrt(eta_loop), axis=0, fill_value="extrapolate", bounds_error=False)
                self.sol.E[:] = 0
                ...
                Control_func3 = self.sol.co_prop(np.flip(np.conj(Control3), axis=0))
                self.sol.solve(Control_func3, method=self.method)
                grad3 = self.grad([Pcopy3, Scopy3], [np.conj(np.flip(np.flip(self.sol.P, axis=0), axis=1)), np.conj(np.flip(np.flip(self.sol.S, axis=0), axis=1))], Control3/self.sol.gamma)
            if self.sol.solved:
                # retrieve from co-system
                self.sol.E[:] = 0
                self.sol.P[:] = 0
                ...
                Control_func2 = self.sol.co_prop(np.conj(np.flip(Control2, axis=0)))
                self.sol.solve(Control_func2, method=self.method)            
                grad2 = self.grad([Pcopy2, Scopy2], [np.conj(np.flip(np.flip(self.sol.P, axis=0), axis=1)), np.conj(np.flip(np.flip(self.sol.S, axis=0), axis=1))], Control2/self.sol.gamma)
            if self.sol.solved:    
                #MZ retrieve
                self.sol.Einits = interp1d(self.sol.tpoints, self.sol.E[:, -1]*np.sqrt(eta_loop), axis=0, fill_value="extrapolate", bounds_error=False)
                self.sol.E[:] = 0
                ...
                Control_func1 = self.sol.co_prop(np.conj(np.flip(Control1, axis=0)))
                self.sol.solve(Control_func1, method=self.method)
                grad1 = self.grad([Pcopy, Scopy], [np.conj(np.flip(np.flip(self.sol.P, axis=0), axis=1)), np.conj(np.flip(np.flip(self.sol.S, axis=0), axis=1))], Control1/self.sol.gamma)
                grads = np.array([grad1, grad2, grad3, grad4])
            else:
                storage_eff = 0
                total_eff = 0
    ...
    def adapt_step_momentum(self, step_sizes, grads, effs, count):
        """
        Adapt the step size using a scalar momentum based approach.
        """

    def Krotov_momentum(self, function, Controls, initial_step_sizes, tol, adaptive_step_size=False, max_energies=False, max_powers=np.array([False]), extra_params=False, max_step_size = False):
        count = 0
        effs = np.zeros(4)
        created_plots_bool = False
        if self._complex:
        ...
        Control_hist[0] = Controls
        grad_hist = np.zeros((4, Control_number, self.sol.m, 2), dtype=dtype)
        momentum_coefficients = np.flip( (1-self.beta1**np.arange(1, 5))/(1-self.beta1) )
        step_sizes = self.set_initial_step_sizes(initial_step_sizes, Control_number)
        self.arrays_for_counter_prop() # does nothing for co_prop
        if max_energies:
            adaptive_step_size = False
        ...
        while count < 4:
            ...
            if self.live_plot or self.save_to_file:
                if count<1:
                    ...
                    if self.save_to_file:
                        # filename = self.generate_filename(self.sol.metadata())
                        pkl_filename, npz_filename = self.generate_filename()
            ...
            if max_energies:
                ...
                Control_hist[count+1] = Control_hist[count] + np.einsum('ncp, nctp -> ctp', 
                                                                        np.einsum('n, cp -> ncp', momentum_coefficients[-(count+1):], step_sizes), grad_hist[0:count+1]*self.sol.gamma)
            count+=1
        ...
        if self.sol.solved:
            while (diff > tol) and below_max_points:
                ...
                if max_energies:
                    ...
                    Control_hist[4] = Control_hist[3] + np.einsum('ncp, nctp -> ctp', 
                                                                        np.einsum('n, cp -> ncp', momentum_coefficients, step_sizes), grad_hist*self.sol.gamma)
                count+=1
                ...
                eff_average = (effs[2] + effs[1] + effs[0])/3
                diff = 2*tol #(effs[3] - eff_average)/eff_average
        ...
        if self.live_plot: #?
            self.update_plots(Control_number, [storage_eff_hist, total_eff_hist, grad_norms_hist, step_sizes_hist, count_hist, Control_hist[index]])
        if self.save_to_file:
            # Save class instance as pkl
            with open(pkl_filename, 'wb') as file:
                pickle.dump(self.sol, file)
            
            # Save the arrays to a file
            np.savez(npz_filename, Control_number=Control_number, storage_eff_hist=storage_eff_hist, total_eff_hist=total_eff_hist, grad_norms_hist=grad_norms_hist, 
                    step_sizes_hist=step_sizes_hist, count_hist=count_hist, Control_best=Control_hist[index], Control_init = Controls)
            # can now load using np.load and the variables will be saved under the identifiers above
